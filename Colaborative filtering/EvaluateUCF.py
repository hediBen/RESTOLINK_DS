# -*- coding: utf-8 -*-
"""
Created on Fri May  3 16:48:00 2019
...
from RecMetrics import RecommenderMetrics
...
for uiid in range(trainSet.n_users):
    ...
    for itemID, ratingSum in sorted(candidates.items(), key=itemgetter(1), reverse=True):
        if not itemID in watched:
            restID = trainSet.to_raw_iid(itemID)
            topN[int(trainSet.to_raw_uid(uiid))].append( (int(restID), 0.0) )
            pos += 1
            if (pos > 40):
                break

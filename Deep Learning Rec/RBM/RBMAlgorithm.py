...
class RBMAlgorithm(AlgoBase):
    ...
    def __init__(self, epochs=10, hiddenDim=100, learningRate=0.001, batchSize=10, sim_options={}):
    ...
    def fit(self, trainset):
        ...
        trainingMatrix = np.zeros([numUsers, numItems, 12], dtype=np.float32)
        ...
        for uiid in range(trainset.n_users):
            ...
            recs = np.reshape(recs, [numItems, 12])
            ...
            for itemID, rec in enumerate(recs):
                ...
                rating = np.average(np.arange(12), weights=normalized)
